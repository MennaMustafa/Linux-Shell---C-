/*Team: Menna-Tullah Mustafa Fathy & Bassel Hossam Shawkat
Assumptions:
1- No space in folders/file names when using cd command
2- only one space between each 2 arguments in one command
3- Ctrl+c doesn't terminate background processes
4- Customized command "terminate" is used to terminate this shell
5- When pressing ctrl c while entering command you have to press any key to get out of getch and then enter the new command from beginning
*/
	...
	do{	//we get chars util the enter is pressed
		...
		if(tx == '\033') { // if the first value is esc
			...
			if(temp.length()>0 && point == temp.length()){
				printf("\b \b");	//first we move the cursor to the left then put a space and then to the left again and wola :D :D
...
int main () {
	...
	while(1){
		...
		//Terminate
		if(In == "terminate "){
			printf("Student Linux Shell terminated\n");
			raise(SIGKILL);
		}

		//Menna's touch :D
		else if(In == "Menna "){
			char**C=ParseCommand("xdg-open https://longawkwardpause.files.wordpress.com/2014/08/grumpy_cat.jpg ");
